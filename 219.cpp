...
int main()
{
	...
	char temp[100];
	cin >> num;
	char **bookNames = (char**)malloc(num * sizeof(char*));
	for (int i = 0; i < num; i++)
	{
		cin >> temp;
		bookNames[i] = (char*)malloc(100 * sizeof(char));
		strcpy(bookNames[i], temp);
	}

	//查找图书信息并输出
	for (int i = 0; i < num; i++)
	{
		BList targetList = bList.searchByName(bookNames[i]);
		if (targetList.length == 0)
			cout << "Sorry，there is no your favourite!" << endl;
		else
		{
			cout << targetList.length << endl;
			targetList.traverseList();
		}
	}
